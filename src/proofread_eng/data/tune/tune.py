...
from extra_word import evaluation_dataset as extra_word_dataset
from grammar import evaluation_dataset as grammar_dataset
from matplotlib.ticker import FixedFormatter, FixedLocator
from missing_word import evaluation_dataset as missing_word_dataset
...
def report_results(
    ...
    for version in versions:
        ...
        model_table_md += f" {proofreader.beta} |"
        ...
        try:
            ...
            # If there is no proofreader.model, then there should be
            # a proofreader.models, a multi-model proofreader
            proofreader.n_books = 0
            for i_model, model in enumerate(proofreader.models):
                # Leave the proofreader information empty for all but
                # the first model row.
                if i_model > 0:
                    model_table_md += "\n|  |"  # version column
                    model_table_md += "  |"  # error_cutoff column
                    model_table_md += "  |"  # beta column

                model_table_md += f" {model.description} |"
                model_table_md += f" {model.version} |"
                model_table_md += f" {model.tokenizer_version} |"
                ...
                n_books = 0
                try:
                    for corpus_version in model.corpus_versions:
                        n_books += corpus_registry[corpus_version].n_files
                except AttributeError:
                    pass
                proofreader.n_books = np.maximum(proofreader.n_books, n_books)
                model_table_md += f" {n_books} |"
    ...
    for version in versions:
        ...
        if label_type == "error_threshold":
            performance_point_labels.append(proofreader.error_threshold)
        ...
        elif label_type == "beta":
            performance_point_labels.append(proofreader.beta)
